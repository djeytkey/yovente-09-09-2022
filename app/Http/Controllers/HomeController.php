...
class HomeController extends Controller
{
    ...
    public function index()
    {
        ...
        if(Auth::user()->role_id > 2 && $general_setting->staff_access == 'own') { // Role Vendeur
            ...
            $en_cours = Delivery::where([['sold_by', Auth::id()], ['is_close', false], ['returned', false]])->count();
            $returned = Delivery::where([['sold_by', Auth::id()], ['is_close', false], ['returned', true]])->count();
            ...
            $livre_facture = Delivery::where('is_close', true)->count();
            $en_cours = Delivery::where([['is_close', false], ['returned', false]])->count();
            $returned = Delivery::where([['is_close', false], ['returned', true]])->count();
        ...
        //return $month;
        return view('index', compact('demande_withdraws', 'total_withdraws', 'reste', 'total_sales', 'livre_facture', 'en_cours', 'returned', 'profit_row', 'revenue', 'purchase', 'expense', 'return', 'purchase_return', 'profit', 'payment_recieved', 'payment_sent', 'month', 'yearly_sale_amount', 'yearly_purchase_amount', 'recent_sale', 'recent_purchase', 'recent_quotation', 'recent_payment', 'best_selling_qty', 'yearly_best_selling_qty', 'yearly_best_selling_price'));
    }
